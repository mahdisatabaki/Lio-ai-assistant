...
const FAILURES: Record<FailureKind, Omit<FailureInfo, "kind">> = {
  "retrieval-unavailable": {
    message:
      "الان نمی‌تونم به منبع مستندات لیارا وصل بشم، و بدون اون ترجیح می‌دم جواب حدسی ندم. چند لحظه دیگه دوباره امتحان کن.",
    retryable: true,
  },
  "embedding-unavailable": {
    message:
      "جست‌وجو در مستندات لیارا موقتاً در دسترس نیست. چون نمی‌خوام بدون منبع جواب بدم، یه بار دیگه تلاش کن.",
    retryable: true,
  },
  "model-unavailable": {
    message:
      "ارتباط با سرویس هوش مصنوعی الان برقرار نشد. متن سؤالت حفظ شده؛ دوباره بفرست.",
    retryable: true,
  },
  "index-empty": {
    message:
      "فهرست مستندات لیارا هنوز ساخته نشده، برای همین جوابی که بشه بهش تکیه کرد ندارم. این یک مشکل سمت سرویسه، نه سؤال تو.",
    retryable: false,
  },
};
